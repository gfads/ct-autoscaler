...
func (p *Prometheus) MakeQuery(query string) float64 {
	client, err := api.NewClient(api.Config{
		Address: p.Protocol + "://" + p.Address + ":" + p.Port,
	})
	if err != nil {
	...
	resultF := convertResultToFloat(result)
	return resultF
}
...
func convertResultToFloat(result model.Value) float64 {
	resultString := strings.TrimSpace(strings.Split(strings.Split(result.String(), "@")[0], ":")[1])
	//fmt.Println(resultString)
	scalarFloat, _ := strconv.ParseFloat(resultString, 32)
	if math.IsNaN(scalarFloat) {
		return 0.0
	}
	return scalarFloat
}
...
/*func main() {
	prom := Prometheus{protocol: "http", port: "9090", address: "localhost", query: `scalar(histogram_quantile(0.90, sum(irate(istio_request_duration_milliseconds_bucket{destination_service=~"frontend-external.default.svc.cluster.local"}[1m])) by (le)) / 1000)`}
	result := prom.makeQuery()
	floatResult := convertResultToFloat(result)
